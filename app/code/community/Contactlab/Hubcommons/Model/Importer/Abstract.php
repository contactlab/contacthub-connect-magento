...
abstract class Contactlab_Hubcommons_Model_Importer_Abstract extends Contactlab_Hubcommons_Model_AbstractImportExport {
    /** Import the xml file. */
    public final function import(Contactlab_Hubcommons_Model_Task_Interface $task) {
        if (!$this->isEnabled()) {
            Mage::helper("contactlab_hubcommons")->logWarn("Module import is disabled");
            return "Module import is disabled";
        }

        if ($this->_useLocalServer()) {
            $filename = $this->getTask()->getConfig("contactlab_hubcommons/connection/import_local_path")
                    . '/' . $this->getFileName();
			$filename = $this->_formatFileName($filename);
	        Mage::helper("contactlab_hubcommons")->logNotice("Importing locally from $filename");
    ...
    private function _getFile($filename) {
        if ($this->_useRemoteServer()) {
			...
			$remoteFile = $this->getTask()->getConfig("contactlab_hubcommons/connection/import_remote_path")
	                . '/' . basename($filename);
			...
			Mage::helper("contactlab_hubcommons")->logNotice("Remote file is $remoteFile");
