 ...
 */
abstract class Contactlab_Hubcommons_Model_Soap_AbstractCall extends Mage_Core_Model_Abstract {
	public function singleCall(Contactlab_Hubcommons_Model_Task $task = NULL) {
	    if (!is_null($task)) {
	       $this->setTask($task);
        }
		$this->startSession();
	...
	public function startSession() {
		...
		$wsdl = $this->getConfig("contactlab_hubcommons/soap/wsdl_url");
		if (empty($wsdl)) {
			throw new Zend_Exception("WSDL url not configured");
		}
		$apikey = $this->getConfig("contactlab_hubcommons/soap/apikey");
		if (empty($apikey)) {
			throw new Zend_Exception("SOAP apikey not configured");
		}
		$userKey = $this->getConfig("contactlab_hubcommons/soap/uid");
		if (empty($userKey)) {
		...
		//Mage::helper("contactlab_hubcommons")->logDebug("SOAP token borrowed");
	}
	...
	public function endSession() {
		...
		//Mage::helper("contactlab_hubcommons")->logDebug("SOAP Token Invalidated");
		return $rv;
	}
	...
	public function isSoapEnabled() {
		return $this->getConfigFlag("contactlab_hubcommons/soap/enable");
	} 
