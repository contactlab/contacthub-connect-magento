 ...
 *
 * @method Contactlab_Hubcommons_Model_Task getTask
 */
abstract class Contactlab_Hubcommons_Model_Exporter_Abstract extends Contactlab_Hubcommons_Model_AbstractImportExport {
    /** Export the xml file.
     * @param Contactlab_Hubcommons_Model_Task_Interface $task
     * @return string
    ...
    public function export(Contactlab_Hubcommons_Model_Task_Interface $task) {
        ...
        if ($this->_useLocalServer()) {
            $filenameFormat = $this->getTask()->getConfig("contactlab_hubcommons/connection/export_local_path")
                    . '/' . $this->getFileName();
        } else {
        ...
        if (is_dir($path) && !is_writable($path)) {
            Mage::helper("contactlab_hubcommons")->logAlert("$path is not writeable");
            throw new Zend_Exception("$path is not writeable");
        }
        if (is_file($filename) && !is_writable($filename)) {
            Mage::helper("contactlab_hubcommons")->logAlert("$filename is not writeable");
            throw new Zend_Exception("$filename is not writeable");
        }
        if (($this->gz = gzopen($filename, 'w9')) === false) {
            Mage::helper("contactlab_hubcommons")->logAlert("Could not export to $filename");
    ...
    private function _putFile($filename, $realFilename) {
        ...
        $remoteFile = $this->getTask()
                ->getConfig("contactlab_hubcommons/connection/export_remote_path")
                . '/' . $realFilename;
