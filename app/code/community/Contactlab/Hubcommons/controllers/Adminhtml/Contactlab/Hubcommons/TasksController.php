 ...
 * Tasks controller.
 */
class Contactlab_Hubcommons_Adminhtml_Contactlab_Hubcommons_TasksController extends Mage_Adminhtml_Controller_Action {
	...
	public function indexAction() {
		...
		// Mage::getModel('contactlab_hubcommons/cron')->checkErrors();
	...
	public function consumeAction() {
		...
		if (!Mage::helper('contactlab_hubcommons')->isAllowed('tasks', 'consume')) {
			$session->addError(Mage::helper('adminnotification')->__('Consume task is not allowed.'));
			$this->_redirect('*/*');
			return;
		}
		Mage::getModel('contactlab_hubcommons/cron')->consumeQueue();
		$this->_redirect('*/*');
	}
	...
	public function suspendAction() {
		$session = Mage::getSingleton('adminhtml/session');
		if (!Mage::helper('contactlab_hubcommons')->isAllowed('tasks', 'suspend')) {
			$session->addError(Mage::helper('adminnotification')->__('Suspend task is not allowed.'));
			$this->_redirect('*/*');
			return;
		}
		try {
			Mage::getModel("contactlab_hubcommons/task")->load($this->getRequest()->getParam("task_id"))->suspend()->save();
	...
	public function deleteAction() {
		$session = Mage::getSingleton('adminhtml/session');
		if (!Mage::helper('contactlab_hubcommons')->isAllowed('tasks', 'delete')) {
			$session->addError(Mage::helper('adminnotification')->__('Delete task is not allowed.'));
			$this->_redirect('*/*');
			return;
		}
		try {
			Mage::getModel("contactlab_hubcommons/task")->load($this->getRequest()->getParam("task_id"))->delete()->save();
	...
	public function clearAction() {
		$session = Mage::getSingleton('adminhtml/session');
		if (!Mage::helper('contactlab_hubcommons')->isAllowed('tasks', 'clear')) {
	...
	public function unsuspendAction() {
		$session = Mage::getSingleton('adminhtml/session');
		if (!Mage::helper('contactlab_hubcommons')->isAllowed('tasks', 'unsuspend')) {
			$session->addError(Mage::helper('adminnotification')->__('Unsuspend task is not allowed.'));
			$this->_redirect('*/*');
			return;
		}
		try {
			Mage::getModel("contactlab_hubcommons/task")->load($this->getRequest()->getParam("task_id"))->unsuspend()->save();
	...
	public function retryAction() {
		$session = Mage::getSingleton('adminhtml/session');
		if (!Mage::helper('contactlab_hubcommons')->isAllowed('tasks', 'retry')) {
			$session->addError(Mage::helper('adminnotification')->__('Retry is not allowed.'));
			$this->_redirect('*/*');
			return;
		}
		try {
			Mage::getModel("contactlab_hubcommons/task")->load($this->getRequest()->getParam("task_id"))->reset()->save();
	...
	public function cancelAction() {
		$session = Mage::getSingleton('adminhtml/session');
		if (!Mage::helper('contactlab_hubcommons')->isAllowed('tasks', 'cancel')) {
			$session->addError(Mage::helper('adminnotification')->__('Cancel task is not allowed.'));
			$this->_redirect('*/*');
			return;
		}
		try {
			Mage::getModel("contactlab_hubcommons/task")->load($this->getRequest()->getParam("task_id"))->cancel()->save();
	...
	public function runAction() {
		$session = Mage::getSingleton('adminhtml/session');
		if (!Mage::helper('contactlab_hubcommons')->isAllowed('tasks', 'run')) {
			$session->addError(Mage::helper('adminnotification')->__('Run task is not allowed.'));
			$this->_redirect('*/*');
			return;
		}
		try {
			Mage::getModel("contactlab_hubcommons/task")->load($this->getRequest()->getParam("task_id"))->runTask()->save();
	...
	public function getRequestStatusAction() {
		$rv = new stdClass();
        // FIXME multi store
		$r = Mage::getModel('contactlab_hubcommons/soap_getSubscriberDataExchangeStatus')->singleCall();
        $rv->status = $r;
        if ($r === 'COMPLETED') {
	...
	public function getStatusAction() {
		...
		$rv = array();
		$coll = Mage::getModel("contactlab_hubcommons/task")->getCollection()
			->addFieldToFilter('task_id', array('in' => $ids));
		foreach ($coll as $task) {
			$i = new stdClass();
			$i->html = Contactlab_Hubcommons_Block_Adminhtml_Tasks_Renderer_Model::renderTask($task);
			$i->statusHtml = Contactlab_Hubcommons_Block_Adminhtml_Events_Renderer_Status::renderTask($task);
			$i->actionsHtml = Contactlab_Hubcommons_Block_Adminhtml_Tasks_Renderer_Actions::renderTask($task);
		...
		foreach ($taskIds as $id) {
			switch ($taskStatus) {
				case 1:
					...
					try {
						Mage::getModel('contactlab_hubcommons/task')->load($id)->cancel()->save();
						++$num;
					} catch (Exception $e) {
				...
				case 2:
					...
					try {
						Mage::getModel('contactlab_hubcommons/task')->load($id)->suspend()->save();
						++$num;
					} catch (Exception $e) {
						$session->addException($e, Mage::helper('adminnotification')->__('An error occurred while changing the status %1$s to %2$s.', $type, $id));
					}
				...
				case 3:
					...
					try {
						Mage::getModel('contactlab_hubcommons/task')->load($id)->unsuspend()->save();
						++$num;
					} catch (Exception $e) {
						$session->addException($e, Mage::helper('adminnotification')->__('An error occurred while changing the status %1$s to %2$s.', $type, $id));
					}
				...
				case 4:
					$type = $this->__('Retry');
					try {
						Mage::getModel('contactlab_hubcommons/task')->load($id)->reset()->save();
						++$num;
					} catch (Exception $e) {
						$session->addException($e, Mage::helper('adminnotification')->__('An error occurred while changing the status %1$s to %2$s.', $type, $id));
					}
				...
				case 5:
					...
					try {
						Mage::getModel('contactlab_hubcommons/task')->load($id)->runTask()->save();
						++$num;
					} catch (Exception $e) {
						$session->addException($e, Mage::helper('adminnotification')->__('An error occurred while changing the status %1$s to %2$s.', $type, $id));
					}
	...
	public function massDeleteAction() {
		...
		foreach ($taskIds as $id) {
			try {
				Mage::getModel('contactlab_hubcommons/task')->load($id)->delete()->save();
				++$num;
			} catch (Exception $e) {
