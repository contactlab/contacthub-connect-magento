 ...
 */
class Contactlab_Hubcommons_Helper_Tasks extends Mage_Core_Helper_Abstract {
    ...
    public function consume() {
        $h = Mage::helper("contactlab_hubcommons");
        $tasks = Mage::getModel("contactlab_hubcommons/task")
        ...
        if ($first) {
		    if (Mage::helper('contactlab_hubcommons')->isDebug()) {
    ...
    public function sendEmail() {
        if (!Mage::getStoreConfigFlag('contactlab_hubcommons/error_email/enable')) {
            return;
        }
        ...
        $templateId = Mage::getStoreConfig('contactlab_hubcommons/error_email/send_email_template',
        	Mage::app()->getStore()->getId());
        $senderEmail = Mage::getStoreConfig('contactlab_hubcommons/error_email/email_sender');
        $senderName = Mage::getStoreConfig('contactlab_hubcommons/error_email/email_sender_name');
        $recipients = explode(';', Mage::getStoreConfig('contactlab_hubcommons/error_email/recipients'));
        ...
        foreach ($recipients as $recipient) {
            ...
            $mailTemplate->sendTransactional(
                    ...
                    array('name' => $senderName,
                            'email' => $senderEmail),
                    trim($recipient),
                    Mage::helper('contactlab_hubcommons')->__('Tasks error notification'),
                    array(),
                    Mage::app()->getStore()->getId()
            );
        }
    ...
    public function updateErrorTaskArray() {
        foreach ($this->getErrorTasks() as $task) {
        	Mage::getModel('contactlab_hubcommons/task_event')->load($task->getTaskEventId())
        		->setEmailSent(1)->save();
        }
    }
    ...
    private function loadErrorTasks() {
        ...
        $tasks = Mage::getModel('contactlab_hubcommons/task')->getCollection();
        $tasks->addFieldToSelect('task_code')
            ...
            ->getSelect()
            ->join(array('task_event' => Mage::getSingleton('core/resource')->getTableName($tablePrefix.'contactlab_hubcommons_task_event_entity')),
    ...
    public function addCriticalMessage($value) {
        Mage::helper('contactlab_hubcommons')->addCriticalMessage($value);
    }
}
