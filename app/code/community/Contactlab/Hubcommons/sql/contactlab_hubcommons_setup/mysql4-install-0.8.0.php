...
$installer->run("drop table if exists {$installer->getTable("contactlab_hubcommons/task")};");

$tablePrefix = (string) Mage::getConfig()->getTablePrefix();

$installer->run(<<<EOT
DROP TABLE IF EXISTS `{$tablePrefix}contactlab_hubcommons_task_entity`;
EOT
);

$installer->run(<<<EOT
CREATE TABLE `{$tablePrefix}contactlab_hubcommons_task_entity` (
  ...
  PRIMARY KEY (`task_id`),
  KEY `IDX_CONTACTLAB_HUBCOMMONS_TASK_ENTT_STATUS` (`status`),
  KEY `FK_CONTACTLAB_HUBCOMMONS_TASK_ENTT_STORE_ID_CORE_STORE_STORE_ID` (`store_id`),
  CONSTRAINT `FK_CONTACTLAB_HUBCOMMONS_TASK_ENTT_STORE_ID_CORE_STORE_STORE_ID` FOREIGN KEY (`store_id`) REFERENCES `{$tablePrefix}core_store` (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT='Tasks';
EOT
);
...
$installer->run(<<<EOT
DROP TABLE IF EXISTS `{$tablePrefix}contactlab_hubcommons_task_event_entity`;
EOT
);

$installer->run(<<<EOT
CREATE TABLE `{$tablePrefix}contactlab_hubcommons_task_event_entity` (
  ...
  KEY `FK_CONTACTLAB_COMMONS_TASK_EVENT_ENTT_TASK_ID` (`task_id`),
  KEY `FK_CONTACTLAB_HUBCOMMONS_TASK_EVENT_ENTT_USR_ID_ADM_USR_USR_ID` (`user_id`),
  CONSTRAINT `FK_CONTACTLAB_COMMONS_TASK_EVENT_ENTT_TASK_ID` FOREIGN KEY (`task_id`) REFERENCES `{$tablePrefix}contactlab_hubcommons_task_entity` (`task_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_CONTACTLAB_HUBCOMMONS_TASK_EVENT_ENTT_USR_ID_ADM_USR_USR_ID` FOREIGN KEY (`user_id`) REFERENCES `{$tablePrefix}admin_user` (`user_id`) ON DELETE SET NULL ON UPDATE CASCADE
) COMMENT='Task events';
EOT
);
...
$installer->run(<<<EOT
DROP TABLE IF EXISTS `{$tablePrefix}contactlab_hubcommons_deleted_entity`;
EOT
);
$installer->run(<<<EOT
CREATE TABLE `{$tablePrefix}contactlab_hubcommons_deleted_entity` (
  ...
  KEY `FK_CONTACTLAB_COMMONS_DELETED_ENTT_TASK_ID` (`task_id`),
  CONSTRAINT `FK_CONTACTLAB_COMMONS_DELETED_ENTT_TASK_ID` FOREIGN KEY (`task_id`) REFERENCES `{$tablePrefix}contactlab_hubcommons_task_entity` (`task_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
$installer->run(<<<EOT
DROP TABLE IF EXISTS `{$tablePrefix}contactlab_hubcommons_log_entity`;
EOT
);
$installer->run(<<<EOT
CREATE TABLE `{$tablePrefix}contactlab_hubcommons_log_entity` (
