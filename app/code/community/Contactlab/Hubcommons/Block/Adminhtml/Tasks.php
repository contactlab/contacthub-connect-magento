 ...
 * Tasks block.
 */
class Contactlab_Hubcommons_Block_Adminhtml_Tasks extends Mage_Adminhtml_Block_Widget_Grid_Container {
    ...
    public function __construct() {
		...
		$this->addCommandButton("tasks", "clear", 'contactlab_hubcommons', "Clear old tasks", 'Are you sure you want to do this?', '*/*/clear');
		if (Mage::helper('contactlab_hubcommons')->isDebug()) {
			$this->addCommandButton("tasks", "consume", 'contactlab_hubcommons', "Run tasks",
					null, '*/*/consume');
		}
    }
    ...
    public function addCommandButton($section, $code, $helper, $label, $alert, $url, $params = array(), $onclickParam = null) {
    	if (!Mage::helper('contactlab_hubcommons')->isAllowed($section, $code)) {
    		return;
    	}
