 ...
 */
class Contactlab_Hubcommons_Block_Adminhtml_Tasks_Renderer_Actions
	...
	public static function renderTask(Contactlab_Hubcommons_Model_Task $task) {
	...
	public static function _addAction(&$rv, Contactlab_Hubcommons_Model_Task $task, $code, $title, $enabled) {
		$s = "";
		$title = Mage::helper('contactlab_hubcommons')->__($title);
		if (!$enabled) {
			$s = "style=\"opacity:0.2; cursor: default;\" ";
		}
		$img = sprintf("<img %ssrc=\"%s\" title=\"%s\" alt=\"%s\"/>",
			$s,
			Mage::getDesign()->getSkinUrl('images/contactlab/hubcommons/' . $code . '.png'),
			$title, $title);
		if ($enabled) {
