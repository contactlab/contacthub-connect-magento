<?php
/*
 * Shamelessly copied from https://github.com/jenssegers/agent
 */
require_once Mage::getBaseDir('lib') . DS . 'Contactlab' . DS .'MobileDetect' . DS . 'Mobile_Detect.php';

class Contactlab_Hub_Model_Mobiledetect extends ContactlabHub_Mobile_Detect
{
    /**
    ...
    public function getMobileBrand() {
        ...
        foreach ($this->getRules() as $k => $_regex) {
            if (empty($_regex)) {
                continue;
            }

            if ($this->match($_regex, null)) {
                return $k;
            }
        }
